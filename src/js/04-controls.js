...
const setRangeFromTouch = (touch)=>{
  ...
  const thumb = matchMedia("(max-width: 700px)").matches ? 24 : 13;
  const f=Math.max(0,Math.min(1,(touch.clientX-box.left-thumb/2)/Math.max(box.width-thumb,1)));
...
function slider(host, key, label, min, max, step, fmt, onAfter){
  ...
  /* Match the browser's inset thumb-center travel at both responsive sizes. */
